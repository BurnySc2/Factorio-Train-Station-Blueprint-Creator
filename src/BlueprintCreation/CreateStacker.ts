...
import type { iBlueprintItem, iBlueprintItemWithoutNumber } from "../constants/interfaces"
import { backLeftCurve, backRightCurve, frontLeftCurve, frontRightCurve } from "../constants/verticalStackerCurves"
import {
...
export const createDiagonalStacker = (bpSettings: typeof defaultSettings): iBlueprintItem[] => {
	...
	const diagonalLength =
		Math.round(
			(2.5 * (doubleHeadedFactor * parseInt(bpSettings.locomotivesPerEnd) + parseInt(bpSettings.cargoWagon))) / 2,
		) *
			2 +
		1
	const parallelTracks = parseInt(bpSettings.stackerNumberParallelLanes)
	...
	const stackerType = validTypes.includes(bpSettings.stackerType) ? bpSettings.stackerType : validTypes[0]
